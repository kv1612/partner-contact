...
def put_lp_pwd(project, lp_entry, password):
    ...
    command = ['lpass', 'add', '--non-interactive', entry_name]
...
def send_pwd_to_lp(pwd, username='admin'):
    ...
    project_name = "cosanum_odoo"
    shortname = "cosanum"
    name = shortname
    loc_prefix = 'http://'
    loc_sufix = '{}.odoo.camptocamp.ch'.format(name)
    location_prod = loc_prefix + loc_sufix
    location_int = loc_prefix + 'integration.' + loc_sufix
    locations = [location_prod, location_int]
    for location in locations:
        entry = make_lp_entry(
            project_name, shortname, name, username, location
        )
