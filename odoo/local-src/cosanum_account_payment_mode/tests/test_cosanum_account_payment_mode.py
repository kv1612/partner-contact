...
class TestCosanumPaymentMode(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.invoice = cls.env["account.move"].create(
            {"partner_id": cls.partner_1.id, "type": "out_invoice"}
        )
    ...
    def test_invoice_partner_bank_set_on_payment_mode_change(self):
        ...
        self.assertEqual(
            self.invoice.invoice_partner_bank_id, self.bank_account_1
        )
        self.invoice.payment_mode_id = self.payment_mode_2
        self.assertEqual(
            self.invoice.invoice_partner_bank_id, self.bank_account_2
        )
    ...
    def test_invoice_partner_bank_in_form(self):
        ...
        self.assertEqual(form.invoice_partner_bank_id, self.bank_account_2)
        form.payment_mode_id = self.payment_mode_1
        self.assertEqual(form.invoice_partner_bank_id, self.bank_account_1)
